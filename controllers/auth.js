      ...
      if (checkusername.length >= 1) {
        ...
        user.save((err,user)=>{
          ...
          token.save(err=>{
            ...
            let mailOptions = {
              from:"no-reply@gstore.cm",
              to:user.email,
              subject:"Account Verification Token",
              text:"Hello,\n\n"+
                "Please verify your account by clicking the link \n http://localhost:3000/api/v1/auth/confirmation/"+
                token.token+
                ".\n"
            };
  ...
  User.findOne({
    ...
    token.save(err => {
      ...
      let mailOptions = {
        ...
        text: "Hello,\n\n" +
          "Please verify your account by clicking the link: \n http://localhost:3000/api/v1/auth/resend/" +
          token.token +
          ".\n"
      };
...
exports.loginUser = async (req, res, next) => {
  ...
  User.findOne({
    ...
    bcrypt.compare(req.body.password, user.passwordHash, (err, isUser) => {
      if (err) {
        res.status(404).json(err);
      }
      if (isUser) {
